...
export default function ProblemGoCrm() {
  ...
  const points = [
    ...
    { 
      num: "04", 
      text: <>"{t.rich("p4", { b: (chunks) => <strong className="font-semibold">{chunks}</strong> })} {t.rich("cierre", { b: (chunks) => <strong className="font-semibold">{chunks}</strong> })}"</> 
    }
  ];
  ...
  return (
    <section ref={container} className="h-screen w-full bg-[var(--color-surface-BG-base)] text-[var(--color-text-primary)] overflow-hidden flex items-center">
      ...
      <div ref={track} className="flex h-full w-max items-stretch">
        ...
        {/* Paneles 1 al 4: Puntos idénticos (excepto el 4 que lleva fondo azul) */}
        {points.map((pt, i) => {
          const isHighlight = i === 3;
          return (
            <div 
              key={i} 
              className={`w-[85vw] lg:w-[28vw] h-full flex flex-col justify-center px-8 lg:px-16 border-r border-[var(--color-border-Strokes-default)] shrink-0 transition-colors duration-500
                ${isHighlight 
              ...
              `}
            >
              <span className={`block text-body-md font-mono mb-12 ${isHighlight ? "text-white/70" : "text-[var(--color-text-muted)]"}`}>
                {pt.num}
              </span>
              <p className={`text-display-xs font-light leading-relaxed ${isHighlight ? "text-white" : "text-[var(--color-text-secondary)]"}`}>
                {pt.text}
              </p>
            </div>
          );
