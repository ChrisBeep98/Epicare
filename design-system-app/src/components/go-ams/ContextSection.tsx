  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.to(obj, {
        ...
        onUpdate: () => {
          if (nodeRef.current) {
            const currentVal = isDecimal ? obj.val.toFixed(1) : Math.floor(obj.val).toString();
            let formatted = currentVal;
            if (!isDecimal) {
              formatted = currentVal.replace(/\B(?=(\d{3})+(?!\d))/g, ",");
            }
...
export default function ContextSection() {
  ...
  const metricsData = [
    { value: "$2.4B", label: "Premium Managed" },
    { value: "12k+", label: "Active Agents" },
    { value: "45s", label: "Avg Quote Time" },
  ...
  return (
    <section 
      ref={sectionRef}
      className="w-full relative z-20 transition-colors duration-500 pt-0 pb-section-sm md:pb-section-md"
    >
      <div className="w-full max-w-section-lg mx-auto px-gutter-sm md:px-gutter-md">
        ...
        <div className="flex md:hidden flex-col mt-0 border-t border-b border-[var(--color-border-Strokes-default)] overflow-hidden w-full max-w-full relative z-10">
           {metricsData.map((m, i) => (
