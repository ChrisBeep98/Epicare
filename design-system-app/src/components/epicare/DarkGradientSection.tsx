...
export default function DarkGradientSection() {
  ...
  const features = [
    { 
      ...
      img: "Dark_wireframe_3D_illustration_on_202606232157.jpeg" 
    },
    { 
      step: "02 · Support", 
      ...
      img: "Dark_wireframe_3D_illustration_on_202606232159 (1).jpeg" 
    },
    { 
      step: "03 · Earnings", 
      ...
      img: "Dark_wireframe_3D_illustration_on_202606232159.jpeg" 
    },
    { 
      step: "04 · Variety", 
      ...
      img: "Dark_wireframe_3D_illustration_on_202606232200.jpeg" 
    }
  ];
  ...
  return (
    ...
    >
      <div className="max-w-section-lg mx-auto w-full">
        ...
        <div className="relative w-full min-h-[60vh] md:min-h-[75vh] rounded-[12px] border border-black/5 dark:border-white/5 overflow-hidden flex flex-col justify-center items-center text-center p-6 md:p-12 lg:p-16 bg-transparent transition-colors duration-500">
          ...
          <div className="relative z-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full mt-4 md:mt-12 perspective-[1000px]">
            {features.map((card, idx) => (
              <div 
                key={idx} 
                className="fade-up flex flex-col rounded-[8px] bg-black/[0.02] dark:bg-white/[0.03] backdrop-blur-xl border border-black/10 dark:border-white/10 hover:bg-black/[0.05] dark:hover:bg-white/[0.06] hover:border-black/20 dark:hover:border-white/20 transition-all duration-500 shadow-sm hover:shadow-[0_16px_48px_rgba(5,72,235,0.15)] hover:-translate-y-2 cursor-pointer group overflow-hidden"
              >
                {/* Bloque de Texto Superior (Step, Título y Cuerpo Largo) */}
                <div className="flex flex-col p-4 md:p-5 w-full gap-3 text-left">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <p className="text-body-sm text-[var(--color-text-Black-100)]/60 dark:text-white/60 font-light leading-relaxed transition-colors duration-500">
                      {card.body}
                    </p>
                  </div>
                </div>

                {/* Contenedor Visual (Medio, flex-1) */}
                <div className="w-full flex-1 min-h-[200px] relative bg-black/5 dark:bg-[#010512] border-y border-black/5 dark:border-white/5 overflow-hidden transition-colors duration-500">
                  <img 
                    src={`/Files/Epicare_Landing/Features/${card.img}`} 
                    alt={card.title} 
                    className="absolute inset-0 w-full h-full object-cover opacity-90 dark:opacity-[0.85] group-hover:opacity-100 group-hover:scale-105 transition-all duration-700 ease-out mix-blend-multiply dark:mix-blend-screen" 
                  />
                </div>

                {/* Subtítulo Corto (Abajo) */}
                <div className="flex flex-col p-4 md:p-5 w-full text-left bg-black/[0.01] dark:bg-white/[0.01] transition-colors duration-500">
                  <p className="text-ui-label md:text-body-sm text-[var(--color-text-Black-100)]/90 dark:text-white/90 font-medium tracking-wide transition-colors duration-500">
                    {card.subtitle}
                  </p>
                </div>
