...
# Replace TypeRow function definition
new_typerow = '''function TypeRow({ token, name, font, text, overrideFont, details }: { token: string, name: string, font: string, text: string, overrideFont?: string, details?: string }) {
  return (
...
content = re.sub(r'function TypeRow\(\{[^\}]+\}.*?return \(\s*<div.*?</p>\s*</div>\s*</div>\s*\);\s*\}', new_typerow, content, flags=re.DOTALL)

# Now redefine the typography section JSX where TypeRow is called
new_typography_jsx = '''
...
print('Updated TypeRow component and typography section with exact CSS specs!')
