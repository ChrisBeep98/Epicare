...
- **132. [NUEVO] Epicare — Rediseño Editorial Footer (30 Jul 2026):** Construido `FooterEpicare.tsx` con estética Awwwards/Swiss Grid. Añadida fluctuación tipográfica para el Hero, letras interactivas inferiores, y animación de "Curtain Reveal" parallax 100% segura usando una tarjeta flotante (`rounded-3xl` + márgenes). Archivo de contexto creado en `sections/footer/context.md`.
- **133. [NUEVO] Epicare — Footer UX Móvil & Arquitectura Grid (31 Jul 2026):** Se refactorizó `FooterEpicare.tsx` a CSS Grid puro, permitiendo reordenamiento asimétrico (Móvil vs Desktop) sin duplicar DOM. Marquee GSAP migrado a dual-track (horizontal fluido en móvil, vertical en desktop). Consolidación estricta de la política Zero Px usando `--space-gutter-sm` (14px) para márgenes móviles. Normalización de jerarquía interactiva (`text-display` / `text-meta`) con subrayados unificados de marca.

### Fase 13 — Hardening de Producción (4 Ago 2026)

- **134. Auditoría de producción completa (5 ejes, rama `prod-hardening`):** censo verificado de arquitectura, i18n, a11y+SEO, performance y disciplina de tokens. Hallazgo raíz nº1: **en Tailwind v4 las utilidades declaradas dentro de `@layer utilities` no generan CSS con variantes**, así que hay **23 usos muertos** (`md:gap-fluid-*` ×12, `md:px-gutter-md` ×4, `hover:shadow-elevation-*` ×7) — nunca se han renderizado. Las migradas a `@utility` (tipografía y `py/pt/pb-section-*`) **sí funcionan**, comprobado contra el CSS compilado. Hallazgo nº2: `ScrollTrigger.refresh()` aparece **una sola vez en todo `src/`** pese a que 4 eventos cambian la altura del documento. Además: `text-display-md` y `text-display-xs` se usan y nunca se declararon; `--color-brand-cyan` y `--color-brand-purple` se usan 20 veces y no están definidos.
- **135. Performance: primera pantalla de ~31 MB → ~6.8 MB.** Los 29 `<video autoPlay>` descargaban ~25 MB de entrada y 13 imágenes sin `loading="lazy"` metían 6.5 MB en la ruta crítica vía el `<link rel="preload">` que emite React 19. Creado `src/components/epicare/SmartVideo.tsx` (`preload="none"` + IntersectionObserver con `rootMargin`), aplicado en Bento desktop y DarkGradient. Borrados 21.2 MB de assets huérfanos (incluido `epicare_landing_hero.mp4` de 19.8 MB, sin referencias), 13 scripts Python de refactors one-shot y `package.json.bak`. `public/`: 56 MB → 35 MB. Cerradas 3 fugas: 8 ScrollTrigger huérfanos en `MetricsEpicare`, un `setTimeout` sin cleanup en `HeaderEpicare` y globals sin consumidores en `LoaderEpicare`.
- **136. Inglés canónico + SEO.** `lang="es"` con cuerpo en inglés → `lang="en"`. Migrados a i18n los 7 Q/A del FAQ, el header de Coverage52 y el CTA/marquee/legales del Footer, que estaban **hardcodeados en español dentro de la página inglesa** (3 namespaces nuevos; 194 claves hoja con paridad exacta; namespace huérfano `whyEpicare` eliminado). SEO pasa de solo `title`+`description` a metadata completa: `metadataBase`, openGraph, twitter, canonical, `robots`, `themeColor`, `sitemap.ts`, `robots.ts` y JSON-LD (Organization + WebSite + FAQPage leyendo las mismas claves que el componente). `/go-ams` recibe metadata propia; antes repetía la de la home.
...
- **141. [NUEVO] Epicare — Bento & Solutions UX/Performance Upgrade (14 Ago 2026):** Rediseño del scroll horizontal del Bento Grid para eliminar fricción (`scrub: true`), tokenización estricta y optimización masiva con `HARDWARE-SYMPHONY` (eliminación de `mix-blend-mode` y blur excesivo). En Agency Solutions se implementó UX Copy persuasivo y premium, se ajustó la paleta a Brand Blue y se inyectó el arte estático `Frame 96.png`.
- **142. [NUEVO] Arquitectura — Resolución Definitiva del Bug "Invalid scope" en Navegación SPA (24 Ago 2026):** Eliminado el flood de 80-100+ errores GSAP en consola al navegar entre rutas (`/` → `/go-ams`). Se corrigió el cleanup huérfano de `ctx.revert()` en 4 componentes (`BentoGrid`, `PeopleReveal`, `ProductLines`, `ProductSpotlight`), se migraron todos los `gsap.context()` y `matchMedia()` para recibir elementos DOM directos en vez de RefObjects nulos, se eliminó el loop de re-creación en `BackOfficeTourB` y se blindó `LoaderEpicare` para soft-navigation. Documentación completa en `session-notes/2026-08-24_gsap-invalid-scope-spa-navigation-fix.md`. Verificado con `tsc --noEmit` (exit 0).
- **143. [NUEVO] GO AMS — Purga de Color Naranja & Refactor de Sección Agente/Agencia (24 Ago 2026):** Se eliminaron todos los acentos naranjas en las secciones de `/go-ams` (`PlatformRevealSection`, `AgentAgencySection`, `DownlineSection`, `ArchitectureSection`, y subcomponentes conceptuales) unificando toda la paleta a Brand Blue (`#35BBFD`). Se refactorizó la sección `AgentAgencySection` invirtiendo el layout: imagen a la izquierda con sangrado `BleedLeft` (0px radius en esquinas izquierdas y curvatura `1.75rem` en esquinas derechas) y mini cards cuadradas con animación de timer a la derecha. Verificado con `pnpm build` (exit 0).
- **144. [NUEVO] GO AMS Landing — Fixes de Deployment Estático y Assets (25 Ago 2026):** Se resolvió un error crítico de Vercel/GitHub Pages (error 404 en imágenes) eliminando la sintaxis JIT arbitraria de Tailwind `bg-[url('...')]` en `PlatformRevealSection.tsx` a favor de estilos inline de React. También se eliminó el uso del componente estricto de servidor `<Image>` de Next.js en `CtaFinalSection.tsx`, migrándolo a un tag HTML nativo `<img>` compatible con static export.
- **145. [NUEVO] Framework & Landing — Protocolo "Zero-Friction Mobile" y UX Táctil (26 Ago 2026):** Resolución masiva de bugs en dispositivos móviles: eliminación del "frenón" de scroll táctil separando las físicas de `scroll-snap` nativo y la librería `GSAP` vía `matchMedia` (deshabilitando los transform/scale dentro de carruseles en `QuoteEnroll` y `HowToJoinSection`), e implementación de CSS nativo para lograr *Bleed-Left* perfecto evadiendo parpadeos por SSR en `AgentAgencySection`. Todo fue documentado en el nuevo `mobile-responsiveness-protocol.md` estandarizando la compresión de márgenes, prohibición de `text-balance` y ergonomía táctil en los CTAs.
- **146. [NUEVO] GO CRM — Arquitectura de Página & Hero Section (1 Sep 2026):** Inicialización de la ruta `/go-crm` con metadata SEO, sitemap estático e i18n sincronizado (EN/ES). Construcción del componente `HeroGoCrm.tsx` con arquitectura de 12 columnas: titular continuo a gran escala ("*Your book of business, one control center.*"), subtítulo refinado con rich text en bold, botón primario oficial de Epicare, microcopy con pulso de estatus y panel de visualización `BleedRight` sangrado a la derecha (`CRM_Hero.png`) con border-radius exclusivo en esquinas izquierdas y sin filtros. Documentado en `session-notes/2026-09-01_go-crm-page-and-hero.md`. Verificado con `tsc --noEmit` (exit 0).

### Fase 14 — Barrido de Producción (7 Sep 2026)

- **147. [NUEVO] Framework — Nuevo protocolo `production-sweep-protocol.md` + cadena de herramientas real (7 Sep 2026):** Barrido profundo ruta-por-ruta y sección-por-sección sobre 5 ejes (assets, código muerto, hardcodeo, i18n, SEO/a11y). Censo → reporte → **STOP** → ejecución; **cuarentena en vez de borrado** (el grep de huérfanos da falsos positivos con rutas construidas en runtime); `design-fingerprint` como prueba de no-regresión. **Descubierto que esta máquina no tiene `winget`, ni Chocolatey, ni Scoop** (Windows 11 IoT Enterprise LTSC) — por eso `optimize-images-prompt.md` y `optimize-video-prompt.md` llevaban tiempo rotos apuntando a un `ffmpeg` en `C:\Users\Grizzly\`. Instalados **ffmpeg/ffprobe 9.0.1** y **libwebp 1.6.0** como binarios estáticos en `%USERPROFILE%\Tools` + PATH de usuario, sin admin. Creados `scripts/asset-audit.mjs`, `optimize-assets.mjs`, `swap-optimized.mjs` y `lib/tools.mjs`, que invocan los CLIs por `child_process` — **cero dependencias npm nuevas**. ⚠️ GitHub Actions no tiene estas herramientas: los assets se optimizan en local y se commitean optimizados.

- **148. [NUEVO] Landing `/` — Barrido de Producción completo, `public/` 73 MB → 25 MB (7 Sep 2026):** Rama `prod-sweep-landing`, 5 commits (uno por eje).
    - ⚠️ **El baseline del fingerprint estaba obsoleto:** daba **1586 diferencias** contra el build actual porque predataba las acciones 142-146. Recongelado desde `main` limpio antes de tocar nada; sin eso la ley del píxel no era verificable. **Actualizado de nuevo al estado post-barrido aprobado.**
    - **Eje A (assets):** vídeo 27.6 → 9.1 MB (**17 de 22 traían pista AAC** en vídeos `muted`; `go_ams_hero` venía a 3826px de ancho para una card → 4.10 MB → 384 KB). Calidad verificada con **SSIM 0.996-0.998 / PSNR ~48 dB**, no asumida. Imágenes 6.9 → 1.0 MB (16 raster a WebP con `cwebp -m 6 -af`). **22 de 22 vídeos con `poster`** (antes 4) y 65 imágenes con `loading="lazy"` — tres salieron del `<link rel="preload">` que emite React 19. Primera pantalla: **5.24 MB**, de los cuales 5.14 MB son el vídeo del hero.
    - **Eje B (código muerto):** 44 assets huérfanos (19.48 MB, el 27% de `public/`) + **11 fondos de un debug panel** que viajaba a producción en `ProductSpotlightEpicare` tras `{false && …}` (sus 5 `useState` estaban congelados → migrados a constantes con valores idénticos) + **4 rutas `/preview-spotlight`** que el propio archivo marcaba como "no debe llegar a producción". Build: 13 → 9 rutas.
    - **Eje C (hardcodeo): NO ejecutado a propósito.** Quedan 70 px arbitrarios, 38 hex y 37 `as any` en las secciones del landing (peores: `BentoGridMobile` 26 `as any`, `LoaderEpicare` 21 hex). Sustituir un `[22px]` por un token **mueve el píxel** si el token no vale exactamente eso → requiere revisión visual.
    - **Eje D (i18n):** paridad en/es **perfecta (380/380 claves)**. Pendiente: 4 `alt` en español en `go-ams/agent-agency/data.ts` dentro de la página inglesa.
    - **Eje E (SEO):** creada la **imagen OG 1200×630** (no existía ninguna: todo link compartido salía sin miniatura), metadata propia + entrada de sitemap para `/licensing` (estaba enlazada desde el header y no tenía ninguna), y arreglado el título duplicado `| Epicare · Epicare` de `/go-crm`.
    - **Refactors de paso:** `posterFor()` extraído a `lib/asset.ts` (3 consumidores); `cardAsset()`/`cardPoster()` en `DarkGradientSection` (la expresión estaba repetida 4 veces); `CierreGoCrm` usaba `src="/Files/..."` **sin `asset()`** → 404 bajo `basePath`.
    - **Verificado:** `tsc --noEmit` limpio, `pnpm build` verde, fingerprint **sin cambios en CLASES, INLINE ni TEXTO** (solo MEDIA -36/+36 y META -3/+5, ambos intencionales), y **las 73 referencias de assets del HTML resuelven a archivo real**.
    - 🐞 **Bug preexistente destapado, NO corregido:** `BentoGridMobile.tsx:210` referencia `MacBook_Pro_16_Front.jpeg`, un archivo que **nunca existió en el repo**. Como esa card tiene `videoDarkFullBackground: true`, la rama de vídeo oscuro se desactiva y se renderiza esa imagen rota → **404 visible en dark mode móvil**. Arreglarlo es decidir entre quitar la clave `image` o aportar el asset.

- **149. [NUEVO] `/go-ams` — Barrido de Producción, `public/` 25 MB → 18 MB (7 Sep 2026):** Rama `prod-sweep-go-ams`, apilada sobre `prod-sweep-landing`. Baseline de la ruta congelado antes de empezar (no existía) y actualizado al estado aprobado.
    - **Eje A:** imágenes 7.5 MB → 872 KB (BackOfficeTour 7 JPEG de 1024×1024 → −87%; `go-ams-quote.jpeg` venía a **2752×1536** para un panel de reveal, 2.23 MB → 143 KB; Downline 3 PNG; el aura de QuoteEnroll 315 → 6 KB). Vídeo: `Delegate_Users.mp4` era **el último de los 22 con pista AAC**, 647 → 225 KB. **Primera pantalla de `/go-ams`: 0.38 MB.**
    - **Dos bugs de producción corregidos:** `DelegateUsersSection` usaba `src="/Files/..."` **sin `asset()`** → 404 garantizado bajo `basePath` (era el 2.º de los 2 del censo; **ya no queda ninguno**), y ese mismo `<video autoPlay>` crudo bajo el fold descargaba entero aunque nadie lo viera → migrado a `<SmartVideo>` con poster.
    - **Deuda del framework que estaba obsoleta:** la tabla de `codebase-architecture-protocol.md` §7 afirmaba que `/go-ams` tenía cuerpo en español bajo `lang="en"` y un usuario ficticio (`"Manuel Depool"`, `HeroSection.tsx:86-87`). **Ambos ya estaban resueltos** — 0 coincidencias en `src/`, `messages/` y el HTML generado.
    - **Verificado:** `tsc` limpio, build verde, CLASES y TEXTO sin cambios, fingerprint de `/` intacto en `TOTAL: 0`. `INLINE -11/+11` inspeccionado uno a uno: son los `background-image:url()` cambiando de extensión — **MEDIA disfrazado de INLINE**, documentado como trampa §9.8 del protocolo.
    - 🔴 **Hallazgo NO corregido:** `QuoteEnroll.tsx:42-44` carga **3 avatares desde `randomuser.me`** en producción — dependencia externa sin control, y son personas ficticias presentadas como clientes.

## 6. Próximos Pasos (To-Do)
- **Decisión abierta de `/go-ams` (acción 149):** los 3 avatares de `randomuser.me` en `QuoteEnroll` — ¿assets propios o se quitan?
- **Decisiones abiertas del barrido del landing (acción 148):** (a) el 404 de `MacBook_Pro_16_Front.jpeg` en dark móvil; (b) `ForWhoEpicare.tsx` es **huérfano** (ninguna ruta lo importa) y arrastra 1.42 MB — ¿se resucita o se purga?, junto con `PeopleReveal` y `Coverage52`, aún comentados en `page.tsx`; (c) `Hero_02.mp4` son 20 s de 1080p30 y quedó en 5.14 MB a CRF 24 — hay **4.34 MB a CRF 26** o **3.51 MB a CRF 28** si se acepta bajar calidad del LCP.
- **Barrer las rutas restantes** con `production-sweep-protocol.md`: `/go-crm` (`CRM_Hero.png` = 3 MB), `/go-ams` (`go-ams-quote.jpeg` = 2.23 MB, los 6 JPEG de `Tour/`) y `/licensing`. Quedan 12.4 MB de raster sin migrar, todo fuera del landing.
...
- ~~Verificar visualmente el comportamiento de los `gap-fluid` en el catálogo de productos en mobile.~~ **RESUELTO (acción 134): no se aplican en absoluto.** `gap-fluid-*` sigue declarado en `@layer utilities`, así que `md:`/`lg:gap-fluid-*` no genera CSS. Caso peor: `ProductLinesEpicare.tsx:140` usa `lg:gap-fluid-md` **sin gap base** → grid de 12 columnas con gutter 0 en desktop. Migrar a `@utility` **cambia el diseño aprobado** → requiere revisión visual, no es un fix silencioso.
- **Deuda de producción abierta** (tabla completa con cifras en `command-prompts/codebase-architecture-protocol.md`), por orden de gravedad:
  1. **No hay ruta de conversión:** los 2 CTA del hero son `<button>` sin `onClick` ni `href`, y hay 22 `href="#"` más. La hamburguesa móvil tampoco tiene handler y es el único acceso a nav en móvil. Bloquea lanzamiento; requiere decidir destinos.
  2. **Los 23 estilos muertos** (acción 134). Activarlos cambia el diseño → revisión visual con César.
  3. **No operable por teclado:** FAQ, acordeones de ForWho, FlipCards y la nav del header son `<div onClick>`. `globals.css` tiene **0** reglas de foco y **0** bloques `prefers-reduced-motion`.
  4. **Contraste en modo claro:** `--color-text-muted` 2.50:1 y `--color-brand-blue` como texto 2.17:1 (es el color de todos los overlines). El modo oscuro está bien.
  5. **Bento desktop con reduced-motion:** el pin vive en una media query `no-preference` sobre una sección `md:h-screen md:overflow-hidden` → **5 de 6 cards inalcanzables**. Es pérdida de contenido.
  6. **47 errores de `no-explicit-any`**, casi todos `(card as any)`. Un tipo `EcosystemCard` compartido los mata y además elimina el 82% de duplicación entre Bento Desktop y Mobile.
  7. ~~**Assets pendientes:** `Hero_02.mp4` = 6.17 MB (es el LCP), los 28 vídeos sin `poster`, y 11 JPEG sobre-codificados. Sin imagen de OG (1200×630) los previews salen sin miniatura.~~ **RESUELTO en el landing (acción 148):** `public/` 73 → 25 MB, 22/22 vídeos con `poster`, OG image creada. `Hero_02.mp4` quedó en 5.14 MB (CRF 24, es el LCP — bajar más es decisión de César). **Quedan 12.4 MB de raster** en `/go-crm`, `/go-ams` y `/licensing`, pendientes de su propio barrido.
  8. ~~**`/go-ams`:** cuerpo aún en español bajo `lang="en"`, y muestra un usuario ficticio en producción (`"Manuel Depool"` / `"Admin"`, `HeroSection.tsx:86-87`).~~ **OBSOLETO (verificado en la acción 149):** ambos ya estaban resueltos — 0 coincidencias de `Depool` y 0 frases en español en `out/go-ams/index.html`. **Lo que sí queda en `/go-ams`:** los 3 avatares de `randomuser.me` en `QuoteEnroll.tsx:42-44`.
