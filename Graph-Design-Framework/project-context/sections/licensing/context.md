...
## 1. Visión General
Esta sección cubre los componentes de la página de Licensing (`LicensingHeroEpicare.tsx`, `InteractiveGlobeEpicare.tsx`, `LoaderEpicare.tsx` y `LicensingGridEpicare.tsx`). Su objetivo principal es mostrar la presencia global de licencias utilizando un motor 3D interactivo (WebGL/react-globe.gl) embebido nativamente como widget en un layout de Hero y Grid.
...
- **Efectos GSAP & Sincronización:** El loader de la página (`LoaderEpicare`), el texto de entrada (`LicensingHeroEpicare`) y el despliegue del globo 3D (`InteractiveGlobeEpicare`) están estrictamente orquestados mediante eventos globales (`epicareLoaderFinished`). Ninguna animación del hero se reproduce hasta que el loader termina y la esfera está montada. Las animaciones del globo (pines) cuentan con soporte *Smart Shutdown* a través de CSS puro que se detiene cuando el componente sale de la pantalla para evitar scroll lag.
...
- **13 Ago 2026:** Orquestación de animaciones de entrada entre el Hero, el Globo y el Loader de la página usando eventos globales. Las siglas de los pines se animaron de forma infinita escalonada, pero para evitar el alto consumo de CPU/Scroll Lag en desktop de tener 52 timelines de GSAP activos, se migró toda la matemática GSAP a CSS Keyframes puros controlados por IntersectionObserver (Smart Shutdown).
...
- **HARDWARE-SYMPHONY (Zero JS):** Para animar infinitamente de forma orgánica las 52 iniciales flotantes en los pines sin causar lag en el scroll (causado por el tick the JS recalculando `matrix3d`), se migró la interpolación a CSS Keyframes y se controló globalmente con una simple clase `.globe-active` manejada por IntersectionObserver, aislando todo el renderizado 100% al GPU.
